...
async function migrate() {
  // Prevent accidental use in dev
  if (process.env.NODE_ENV !== 'production') {
    console.error('❌ This migration script is intended for production only.')
    process.exit(1)
  }
  ...
  try {
    ...
    const migrationsDir = path.join(__dirname, 'migrations_prod')
